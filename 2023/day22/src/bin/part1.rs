...
fn solution(s: &str) -> String {
    ...
    // Determine all the blocks that are supported by only one other block.
    // Those blocks cannot be removed.
    let mut immune = HashSet::new();
    for (block, sups) in &supports {
        if sups.len() == 1 {
            immune.insert(sups[0]);
        }
    }

    let ans = blocks.len() - immune.len();
    println!("{ans}");
